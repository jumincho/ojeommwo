...
- **先备好候选，再发送。** 调研和发送是两个独立的任务，搜索失败不会导致发送失败。每餐之前，机器人都会重新核验价格、外卖证据、距离、冷却期和多样性；只有已验证的候选不够时才上网搜索，另外在 11:35 进行一次寻找新餐厅的可选探索。
- **口味模型。** 每道菜都有一个从 Beta(3, 3) 先验出发的 Beta 后验分布。实际用餐的权重为 1.0，问卷评分为 0.9，证据按 180 天的半衰期衰减，并以 18% 的探索率不断引入新选项。同一个人的重复投票会被削弱：同一天只计最新一票，不同日期只计最近三天，权重依次为 1、0.5 和 0.25。结果只是排序信号，并不保证满意。
...
`corepack pnpm test` 也可以运行，但部分测试需要用运营数据库重建快照，而运营数据库不在本仓库中，所以这些测试在这里会失败。
