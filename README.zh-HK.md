...
- **先準備候選，再發送。** 搜集和發送是兩項獨立工作，可選探索失敗時會保留仍然有效的已備候選。每餐之前，機械人都會重新核實價錢、外賣證據、距離、冷卻期和多樣性；只有已驗證的候選不足時才會上網搜尋，另外在 11:35 進行一次尋找新餐廳的可選探索。
- **口味模型。** 每道菜都有一個由 Beta(3, 3) 先驗出發的 Beta 後驗分佈。實際用餐的權重為 1.0，問卷評分為 0.9，證據按 180 天的半衰期遞減，並以 18% 的探索率持續引入新選擇。同一人的重複投票會被減弱：同一天只計最新一票，不同日子只計最近三天，權重依次為 1、0.5 和 0.25。用餐記錄只取代同日及更早的問卷，之後的問卷仍然有效，並逐步減弱較早用餐記錄的影響。結果只是排序訊號，並不保證滿意。
...
`corepack pnpm test` 在沒有私有營運數據庫時使用去識別化樣本及合成數據。公開副本會明確略過一項真實營運儲存整合檢查，該檢查會在伺服器執行。
