...
for (const extension of ["cjs", "mjs"]) {
  test("image-size " + extension + " terminates on zero-size ICNS/HEIF/JXL and keeps valid images", () => {
    ...
    const source = [
      ...
      'for (const length of [1, 7, 0xffffffff]) { const bad = Buffer.from(zeroIcns); bad.writeUInt32BE(length, 12); try { imageSize(bad); } catch (error) { assert.ok(error instanceof Error); } }',
      ...
      'const heif = Buffer.concat([box("ftyp",Buffer.from("heic0000")),box("meta",Buffer.concat([Buffer.alloc(4),box("iprp",box("ipco",box("ispe",dimensions)))]))]);',
      'assert.equal(imageSize(heif).width,37); assert.equal(imageSize(heif).height,23);',
      // Upstream may reject a zero-length EOF box; the security contract is termination.
      'const zeroHeif = Buffer.from(heif); zeroHeif.writeUInt32BE(0,44); try { imageSize(zeroHeif); } catch (error) { assert.ok(error instanceof Error); }',
