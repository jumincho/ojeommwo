...
test("v2.6 package and immutable release metadata agree", () => {
  const pkg = JSON.parse(fs.readFileSync(new URL("../package.json", import.meta.url), "utf8"));
  assert.equal(`${SERVICE_VERSION}.0`, pkg.version);
  assert.equal(SERVICE_VERSION, "2.6");
  assert.match(RELEASE_DATE, /^2026-09-23T\d{2}:\d{2}:\d{2}\+09:00$/u);
  assert.ok(Number.isFinite(Date.parse(RELEASE_DATE)));
  assert.equal(RELEASE_IMPLEMENTATION_MODEL, "GPT-6 Sol Max (Daybreak Blue)");
